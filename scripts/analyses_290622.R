...
phage_cspl <- ggplot(plas_ph, aes(x = species, y = logpfus, col = plasmid, fill = plasmid, group = interaction(plasmid, species)))  +
  ...
  scale_x_discrete(labels = c("ORM_20", "CHF7MC", "VAC_51"))
...
anc_reso <- anc_reso[-11,] #errorenous data point - does not make sense and original result cannot be found in records. Results remain consistent
